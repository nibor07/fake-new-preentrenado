...
def main(args, train_loss_set, val_loss_set):
    ...
    train_loss_set = []
    argument_path = f'{save}/model{sa_num}.json'
    argument = {'seed': seed, 'learning_rate': learning_rate, \
                'batch_size': batch_size, 'epoch': num_epochs, 'max_grad_norm': max_grad_norm, \
                'factor': factor, 'mode': sched_mode, 'patience': patience}
    ...
    # Data load
    train = pd.read_json(f"{train_path}/train.json")
    val = pd.read_json(f"{val_path}/val.json")

    # train data loader
    train_data = Dataset(train, image_path)
    train_dataloader = DataLoader(train_data, batch_size=batch_size)

    # val data loader
    val_data = Dataset(val, image_path)
    ...
    for e_idx in range(1, num_epochs + 1):
        print('Epoch {}/{}'.format(e_idx, num_epochs))
        print('----------------------')
        ...
        for batch_index, batch in enumerate(val_dataloader):
            ...
            with torch.no_grad():
                y_pred = torch.sigmoid(
                    model(input_ids=b_input_ids, attention_mask=b_attention_mask, pixel_values=b_pixel_values))
    ...
    torch.save(best_model_state_on_dev, f'{save}/model{sa_num}.pt')
